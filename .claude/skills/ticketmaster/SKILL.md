...
One record per ticket, one record per line, in ascending ticket order. `<base-branch-name>` is the name `prd-<ticket-number>` of the base branch created for that ticket during Step 2a (e.g., `prd-1`), and `<pr-number>` is the pull request number opened (or already existing) for that ticket's requirements branch.
