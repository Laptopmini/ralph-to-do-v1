...
- [ ] Implement deleteTodo function. Create exported function `deleteTodo(todos: Todo[], id: string): Todo[]` that returns new array with item matching `id` removed; if no item matches, return array unchanged without calling saveTodos. `[test: npx jest tests/unit/implement-deletetodo-function.test.ts]`
